...
- 摘要内容：日期 + 每个账号成功/失败（**账号 UID 打码**，如 `109***660`）+ 耗时，
  只保留最近 30 天，避免仓库膨胀。
- **单账号超时与补跑**：账号日常超过 `account_timeout_minutes`（默认 **15 分钟**）即强杀小助手与游戏、
  换下一个账号；全部账号跑完后，对未成功的账号**补跑一次**（同样受该超时约束）。
  补跑仍失败才记为失败，本次运行不再重试。
  - 页面上的耗时取**最后一次尝试**：补跑成功时显示的是补跑那次的时间。
  - 解析脚本以块头区分首次与补跑：`---------- 账号 X ----------` / `---------- 账号 X (补跑) ----------`。
